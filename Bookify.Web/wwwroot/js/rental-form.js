...
$(document).ready(function () {
    $('.js-search').on('click', function (e) {
        ...
        if (selectedCopies.find(c => c.serial == serial)) {
            showErrorMessage("You can't add the same copy twice");
            return;
        }


        if (selectedCopies.length >= maxAllowedCopies) {
            showErrorMessage(`You can't add more than ${maxAllowedCopies} book(s)`);
            return;
        }
    ...
    $('body').delegate('.js-remove', 'click', function () {
        ...
        prepareInput();
        //hide save button, if copies are zero
...
function onAddCopySuccess(copy) {
    ...
    if (selectedCopies.find(c => c.bookId == bookId)) {
        showErrorMessage("You can't add more than one copy for the same book");
        return;
    } 
